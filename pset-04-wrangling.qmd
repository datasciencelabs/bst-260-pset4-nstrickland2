...
regions <- url |>
  fromJSON(simplifyDataFrame =FALSE) |> # use jsonlite JSON parser
  map_df(as.data.frame) |> # get data as a dataframe
  rename(state_name = states) |>
  unnest(c(state_name, region)) |> # display the elements in the nested lists
  mutate(region_name = case_when(region_name == "New York and New Jersey, Puerto Rico, Virgin Islands" ~ "Other", TRUE ~ region_name)) # keep all other region names unchanged 
...
cases_raw <- request(api) |> # get HTTP GET request
  ...
  resp_body_json(simplifyDataFrame=TRUE) # convert into data frame

nrow(cases_raw) # not all cases are in the dataset
```
...
cases_raw <- request(api) |> # form request
  ...
  req_perform() |> # send request to CDC website
  resp_body_json(simplifyDataFrame=TRUE) |> # convert into dataframe
...
deaths <- request(deaths_url) |> # creates HTTP GET request
  ...
  resp_body_json(simplifyDataFrame = TRUE) |> # returns data as a dataframe
  drop_na(covid_19_deaths, end_date) |>
  filter(state != "United States" & state != "New York City") |> # filter out U.S. and New York City from state variable
